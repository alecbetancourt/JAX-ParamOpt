...
from jaxparamopt.structure import AngleRestraint, BondRestraint, Structure, TorsionRestraint
from jaxparamopt.trainingdata import (
  AngleItem,
  ChargeItem,
  DistItem,
  EnergyItem,
...
def _read_bgf_geometries(geo_file, name_to_index_map, ff_type, far_nbr_cutoff):
  ...
  for line in f:
    ...
    if line.startswith("END"):
      ...
      if len(molcharge_items) > 1 or (
        len(molcharge_items) == 1
        and (molcharge_items[0][1] - molcharge_items[0][0] + 1) < num_atoms
      ):
        print(
          "[ERROR] error in {}, MOLCHARGE is only supported for the total system charge!".format(
            system_name
          )
        )
        sys.exit()
      ...
      # TODO recording atomic numbers for AMBER probably isn't necessary, but double check
      if ff_type == "reaxff":
        atomic_nums = onp.where(atom_names == "C", 6, atomic_nums)
        atomic_nums = onp.where(atom_names == "O", 8, atomic_nums)
        atomic_nums = onp.where(atom_names == "H", 1, atomic_nums)
        ...
        # if the map is a list, then each of the structures corresponds to the same
        # index in the map list
      ...
      # restraint info
        ...
        # d_dist = split_line[5]
        bond_restraints.append(
          [int(at1) - 1, int(at2) - 1, float(force1), float(force2), float(dist)]
        )
    ...
    with h5py.File(train_in, "r") as hf:
      ...
      if "training/energy_items" in hf:
        for i in range(len(hf["/training/energy_items/energies"])):
          name_list = [
            name.decode(encoding="utf-8") for name in hf["/training/energy_items/names"][i]
          ]
  ...
  for line in f:
    ...
    if group_flag:
      ...
      # elif mode == "multi":
        ...
        raise ValueError(
          "Error in reading group item from parameter file, should either be 3 or 4 columns"
        )
  ...
  for p in params:
    ...
    if len(p) == 4:  # group item
      ...
      if isinstance(index_map, list):
        raise ValueError(
          "Error in parameter mapping, cannot have single ff item with multiple ff index map"
        )
      key = (p[0], p[1], p[2])
      ...
      if not isinstance(index_map, list):
        raise ValueError(
          "Error in parameter mapping, cannot have multi ff item with single ff index map"
        )
...
def structure_training_data(training_items, geo_name_to_index):
  ...
  if "energy_items" in training_items:
    ...
    for i, item in enumerate(energy_items):
      ...
      energy_items[i] = dataclasses.replace(energy_items[i], multip=multip, sys_inds=sys_inds)
    training_items["energy_items"] = energy_items
